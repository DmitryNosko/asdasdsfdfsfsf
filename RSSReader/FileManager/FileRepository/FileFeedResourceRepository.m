...
- (FeedResource *) addFeedResource:(FeedResource *) resource {
    ...
    NSString* filePath = [documentDirectory stringByAppendingPathComponent:MENU_FILE_NAME];
    
    if ([self.fileManager fileExistsAtPath:filePath]) {
        
        NSMutableArray<FeedResource *>* decodedResources = [self feedResources];
...
- (void) removeFeedResource:(FeedResource *) resource {
    NSMutableArray<FeedResource *>* resorces = [self feedResources];
    ...
    [self removeAllObjectsFormFile:MENU_FILE_NAME];
    
    for (FeedResource* fR in [resorces copy]) {
        [self addFeedResource:fR];
    }
}
