...
@property (strong, nonatomic) NSMutableDictionary<NSURL*, FeedResource*>* selectedFeedResourcesByURL;
...
@property (strong, nonatomic) NSMutableArray<FeedResource *>* selectedFeedResources;
@end
...
- (void)viewDidLoad {
    ...
    [self tableViewSetUp];
    [self settingsViewSetUp];
    [self timaRangeControlSetUp];
    
    
    
    self.displayedFeeds = [[NSMutableArray alloc] init];
    self.parsedFeeds = [[NSMutableArray alloc] init];
    FeedResource* defaultResource = [[SQLFeedResourceService sharedFeedResourceService] resourceByURL:[NSURL URLWithString:URL_TO_PARSE]];
    
    if (!defaultResource) {
        defaultResource = [[SQLFeedResourceService sharedFeedResourceService] addFeedResource:
                           [[FeedResource alloc] initWithName:TUT_BY_NEWS_FILE_NAME url:[NSURL URLWithString:URL_TO_PARSE]]
                           ];
    }
    
    self.selectedFeedResourcesByURL = [[NSMutableDictionary alloc] initWithObjectsAndKeys:defaultResource, defaultResource.url, nil];
    self.selectedFeedResources = [[NSMutableArray alloc] initWithObjects:defaultResource, nil];
    
    //self.readedItemsLinks = [[FileManager sharedFileManager] readStringsFromFile:READED_NEWS];
    self.readingCompliteItemsLinks = [[SQLFeedItemService sharedSQLFeedItemService] readingCompliteFeedItemLinks:self.selectedFeedResources];
    
    //self.readingInProgressItemsLinks = [[FileManager sharedFileManager] readStringsFromFile:READING_IN_PROGRESS];
    self.readingInProgressItemsLinks = [[SQLFeedItemService sharedSQLFeedItemService] readingInProgressFeedItemLinks:self.selectedFeedResources];
    ...
    if ([ReachabilityStatusChecker hasInternerConnection]) {
        ...
        self.displayedFeeds = [[SQLFeedItemService sharedSQLFeedItemService] feedItemsForResource:defaultResource];
        //self.displayedFeeds = [[FileManager sharedFileManager] readFeedItemsFile:TUT_BY_NEWS_FILE_NAME];
    }
    
    [self itemsWasLoadedHandlerMethod];
    [self feedResourceWasChosenHandlerMethod];
    [self feedResourceWasAddedHandlerMethod];
    [self fetchButtonWasPressefHandlerMethod];
    [self feedResourceWasDeletedHandlerMethod];
}

- (void) viewWillAppear:(BOOL) animated {
    [super viewWillAppear:animated];
    //self.displayedFeeds = [[FeedItemService sharedFeedItemService] feedItems];
    self.favoriteItemsLinks = [[SQLFeedItemService sharedSQLFeedItemService] favoriteFeedItemLinks:self.selectedFeedResources];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([ReachabilityStatusChecker hasInternerConnection]) {
        ...
        FeedItem* item = [self.displayedFeeds objectAtIndex:indexPath.row];
        FeedResource* resource = [self.selectedFeedResourcesByURL objectForKey:item.resourceURL];
        item.isReadingInProgress = YES;
        //item.resource = resource;
        NSThread* thread = [[NSThread alloc] initWithBlock:^{
            [[SQLFeedItemService sharedSQLFeedItemService] updateFeedItem:item atIndex:indexPath.row forResource:resource];
...
- (void)didTapOnDoneButton:(UIBarButtonItem *)doneButton {
    ...
    NSThread* thread = [[NSThread alloc] initWithBlock:^{
        //[[FileRepository sharedFileManager] saveString:self.listenedItem.link toFile:READED_NEWS];
        //[[FileRepository sharedFileManager] removeFeedItem:self.listenedItem fromFile:[NSString stringWithFormat:@"%@%@", resource.name, TXT_FORMAT_NAME]];
    }];
    
    [thread start];
    [self.readingCompliteItemsLinks addObject:self.listenedItem.link];// same with file
    item.isReadingComplite = YES;
    [[SQLFeedItemService sharedSQLFeedItemService] updateFeedItem:item atIndex:0 forResource:resource];
...
- (void) configureNavigationBar {
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"Icon-29"] style:UIBarButtonItemStyleDone target:self action:@selector(chooseNewsDate)];
    self.navigationItem.rightBarButtonItem.tintColor = [UIColor whiteColor];
    
}

- (void) timaRangeControlSetUp {
...
- (void)motionBegan:(UIEventSubtype)motion withEvent:(UIEvent *)event {
    ...
    [self.displayedFeeds removeAllObjects];
    [self.parsedFeeds removeAllObjects];
    ...
    for (NSURL* url in [self.selectedFeedResourcesByURL allKeys]) {
...
- (void) feedResourceWasAddedHandlerMethod {
    ...
    self.feedResourceWasAddedHandler = ^(FeedResource *resource) {
        ...
        [weakSelf.selectedFeedResourcesByURL setObject:resource forKey:resource.url];
...
- (void) feedResourceWasChosenHandlerMethod {
    ...
    self.feedResourceWasChosenHandler = ^(FeedResource *resource) {
        ...
        //FeedItemService* service = stratigi == sql ? [FeedItemService sharedSQLFeedItemService] : [FeedItemService sharedFileFeedItemService];
        //NSMutableArray<FeedItem*>* items = [service feedItemsForResource:resource];
        NSMutableArray<FeedItem*>* items = [[SQLFeedItemService sharedSQLFeedItemService] feedItemsForResource:resource];
        weakSelf.displayedFeeds = items;
        [weakSelf.tableView reloadData];
    };
}

- (void) feedResourceWasDeletedHandlerMethod { // delete resource from selectedFeedResourcesByURL
     __weak MainViewController* weakSelf = self;
    self.feedResourceWasDeletedHandler = ^(FeedResource *resource) {
        [weakSelf.selectedFeedResourcesByURL removeObjectForKey:resource.url];
    };
}

- (void) addParsedFeedItemToFeeds:(FeedItem* ) item {
    if (item) {
        item.resource = [self.selectedFeedResourcesByURL objectForKey:item.resourceURL];
...
- (void) itemsWasLoadedHandlerMethod {
    __weak MainViewController* weakSelf = self;
    ...
    self.rssParser.parserDidEndDocumentHandler = ^{
        ...
        //FeedResource* resource = [weakSelf.selectedFeedResourcesByURL objectForKey:item.resourceURL];
        ...
        [[SQLFeedItemService sharedSQLFeedItemService] cleanSaveFeedItems:weakSelf.displayedFeeds];
